  ...
  persist(
      ...
      batchDelete: () =>
        set((state) => {
          const newPapers = state.papers.filter(
            (p) => !state.checkedItems.has(p.id),
          );
          const newReports = state.reports.filter(
            (r) => !state.checkedItems.has(r.id),
          );
          let newSelected = state.selectedPaper;
          if (
            state.selectedPaper &&
            state.checkedItems.has(state.selectedPaper.id)
          ) {
            newSelected = null;
          }
          return {
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // selectedPaper, checkedItems, activeTab은 persist하지 않음 (세션별 상태)
      }),
    },
  ),
);
